...
function Jobitem({ item, index, total }) {
  const { setSelectedJob, selectedJob } = useJobProvider();
  ...
  let borderClasses =
    index === 0 ? "border-b-2 border-gray-100" : "border-y-2 border-gray-100";

  if (isSelected) {
    borderClasses = "border-2 border-gray-100 rounded-md";
  } else {
    if (
      index < total - 1 &&
      selectedJob?.job_id === data?.[index + 1]?.job_id
    ) {
      borderClasses = "border-t-2 border-gray-100";
    }
    if (index > 0 && selectedJob?.job_id === data?.[index - 1]?.job_id) {
      borderClasses = "border-b-2 border-gray-100";
    }
  }

  return (
    <li
      className={`w-full grid grid-rows-[auto_1fr_auto] gap-y-2 p-2 py-4 cursor-pointer ${borderClasses} hover:bg-gray-100`}
      onClick={handleJobSelected}
    >
